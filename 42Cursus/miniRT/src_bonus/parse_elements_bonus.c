...
int	bonus_add_cone(char **t, t_scene *s)
{
	...
	o->type = OBJ_CONE;
	o->next = s->objects;
	if (!bonus_parse_vec(t[1], &o->cn.center) || !bonus_parse_vec(t[2],
			&o->cn.axis))
		return (free(o), 0);
...
int	bonus_try_checker_plane(char **t, t_scene *s)
{
	...
	if (ft_strncmp(t[0], "pl", 3) || split_len(t)
		!= 7 || ft_strncmp(t[3], "chk", 4))
		return (-1);
	o = malloc(sizeof(*o));
	if (!o)
		return (0);
	o->type = OBJ_PLANE;
	o->next = s->objects;
	if (!bonus_parse_vec(t[1], &o->pl.point) || !bonus_parse_vec(t[2],
			&o->pl.normal))
		return (free(o), 0);
