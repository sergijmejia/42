...
/*   Updated: 2025/10/28 03:17:29 by rafaguti         ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */
...
/**
 * @brief Returns the value of a variable from the parser's temporary variable
 * list.
 *
 * Iterates through the list of parser variables (t_list containing t_pair)
 * and returns the value of the first node whose name matches `name`.
 *
 * @param parser_tmp_var Pointer to the head of the parser's temporary variable
 * list.
 * @param name Name of the variable to find.
 * @return Pointer to the value string if found, NULL otherwise.
 */
char	*get_parser_tmp_var_value(t_list *parser_tmp_var, const char *name)
{
	...
	{
		p = (t_pair *)curr->content;
		if (p && p->name && ft_strncmp(p->name, name,
				ft_strlen(name) + 1) == 0)
			return (p->value);
		curr = curr->next;
	}
